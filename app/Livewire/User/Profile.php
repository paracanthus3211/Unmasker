...
class Profile extends Component
{
    ...
    // Profile Information
    public $name;
    public $username; // ✅ TAMBAH USERNAME
    public $email;
    public $phone;
    public $birth_date;
    ...
    public function mount()
    {
        ...
        $this->name = $user->name;
        $this->username = $user->username; // ✅ TAMBAH USERNAME
        $this->email = $user->email;
        $this->phone = $user->phone;
        $this->birth_date = $user->birth_date ? $user->birth_date->format('Y-m-d') : null;
        $this->selectedAvatar = $user->avatar_id;
        
        // Load available avatars: default + user's custom avatars
        $this->availableAvatars = Avatar::where(function($query) {
            $query->where('is_active', true)
                  ->whereNull('created_by'); // Default avatars
        })->orWhere('created_by', Auth::id()) // User's custom avatars
    ...
    public function updateProfile()
    {
        ...
        $this->validate([
            ...
            'username' => 'required|string|max:255|alpha_dash|unique:users,username,' . $user->id, // ✅ VALIDASI USERNAME
    ...
    public function uploadAvatar()
    {
        ...
        try {
            ...
            $name = pathinfo($originalName, PATHINFO_FILENAME) . '_user_' . Auth::id();
            ...
            $avatar = Avatar::create([
                ...
                'is_active' => true,
                'created_by' => Auth::id(), // Mark as user's custom avatar
            ]);

            // Select the new avatar
            $this->selectedAvatar = $avatar->id;
            Auth::user()->update(['avatar_id' => $avatar->id]);
            ...
            session()->flash('message', 'Avatar uploaded and selected successfully! 🎉');
    ...
    public function deleteAvatar($avatarId)
    {
        ...
        if ($avatar->created_by !== Auth::id()) {
            session()->flash('error', 'You can only delete your own custom avatars!');
            return;
        }
        ...
        try {
            ...
            if ($this->selectedAvatar == $avatarId) {
                ...
                session()->flash('message', 'Custom avatar deleted. Default avatar selected.');
            } else {
                session()->flash('message', 'Custom avatar deleted successfully!');
            }
